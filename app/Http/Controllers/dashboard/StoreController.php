...
class StoreController extends Controller
{
    ...
    // *** طرق جديدة لمعالجة عودة من Stripe ***
    public function stripeRefresh($store_id)
    {
        // إعادة توليد الرابط إذا فشل (مثل انتهاء الصلاحية)
        $store = Store::findOrFail($store_id);
        $stripe = new StripeClient(config('services.stripe.secret_key'));
        $accountLink = $stripe->accountLinks->create([
            'account' => $store->stripe_account_id,
            'refresh_url' => route('dashboard.stripe.refresh', $store->id),
            'return_url' => route('dashboard.stripe.return', $store->id),
            'type' => 'account_onboarding',
        ]);
        return redirect($accountLink->url);
    }

    public function stripeReturn($store_id)
    {
        $store = Store::findOrFail($store_id);
        $stripe = new StripeClient(config('services.stripe.secret_key'));
        $account = $stripe->accounts->retrieve($store->stripe_account_id);

        // تحقق إذا اكتمل الـ Onboarding
        if ($account->charges_enabled && $account->payouts_enabled) {
            $store->update(['stripe_status' => 'active']);
            return redirect()->route('dashboard.')->with('success', 'تم ربط المتجر مع Stripe بنجاح!');
        } else {
            $store->update(['stripe_status' => 'restricted']);
            return redirect()->route('dashboard.')->with('warning', 'الرجاء إكمال إعدادات Stripe لتفعيل الدفعات.');
        }
    }

    public function dashboardStripeLink($store_id)
    {
        $store = Store::findOrFail($store_id);
        $stripe = new StripeClient(config('services.stripe.secret_key'));
        ...
        if ($account->charges_enabled && $account->payouts_enabled) {
            // إذا كان الحساب مفعلاً، استخدم createLoginLink
            $loginLink = $stripe->accounts->createLoginLink($store->stripe_account_id);
            return redirect($loginLink->url);
        } else {
            // إذا كان لا يزال مقيدًا، استخدم account_onboarding
            $accountLink = $stripe->accountLinks->create([
                'account' => $store->stripe_account_id,
            ...
            return redirect($accountLink->url)->with('warning', 'الرجاء إكمال الإعدادات المطلوبة في Stripe.');
        }
    }
}
